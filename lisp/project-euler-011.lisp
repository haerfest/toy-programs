...
(defparameter row-count
  (length grid))

(defparameter col-count
  (length (car grid)))
...
(defun cell (row col)
  "Returns the number at row and column, nil when out of bounds."
  (if (and (>= row 0) (< row row-count)
           (>= col 0) (< col col-count))
    (nth col (nth row grid))
    nil))

(defun diagonal-row (row dir)
  "Returns the numbers at the diagonal starting at row, moving in dir, which
...
(defun diagonal-col (col dir)
  "As diagonal-row, starting at col."
  (let ((row (if (eq dir 'right)
               (- col)
               (- (1+ col) col-count))))
  (diagonal-row row dir)))

(defun max-product (list)
  "Returns the maximum product of four sequential numbers in list."
  (loop for (a b c d) on (append '(0 0 0 0) list)
        while (and b c d)
        maximize (* a b c d)))
