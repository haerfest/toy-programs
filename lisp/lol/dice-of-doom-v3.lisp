             ...
             (tag center ()
               ...
               (let ((chosen (assoc 'chosen params)))
                 (when (or (not *cur-game-tree*) (not chosen))
                   (setf chosen nil)
                   (web-initialize))
                 (cond ((lazy-null (tree-moves *cur-game-tree*))
                        (web-announce-winner (tree-board *cur-game-tree*)))
                       ((zerop (tree-player *cur-game-tree*))
                        (web-handle-human
                         (when chosen
...
(defun web-handle-human (pos)
  (cond ((not pos) (princ "Please choose a hex to move from:"))
        ((eq pos 'pass) (setf *cur-game-tree*
                              (cadr (lazy-car (tree-moves *cur-game-tree*))))
                    ...
                    (take-all (if selected-tile
                                  (lazy-mapcar (lambda (move)
                                                 (when (eql (caar move)
                                                   ...
                                                   (cadar move)))
                                               (tree-moves tree))
                                  (lazy-mapcar #'caar (tree-moves tree)))))))
