...
(defun board-attack-fail (board player src dst dice)
  (declare (ignore dst) (ignore dice))
  (board-array (loop for pos from 0
                  for hex across board
                  collect (if (eql pos src)
                              (list player 1)
                              hex))))
...
(defun handle-computer (tree)
  (let ((ratings (get-ratings (limit-tree-depth tree *ai-level*) (tree-player tree))))
    (pick-chance-branch
     (tree-board tree)
     (lazy-nth (position (apply #'max ratings) ratings) (tree-moves tree)))))
                 ...
                 (lambda (move)
                   (let ((path (car move)))
                     (if path
                         (let* ((src (car path))
                                (dst (cadr path))
                                (probability (aref (aref *dice-probability* (1- (dice dst)))
                                                   (- (dice src) 2))))
